...
function __besman_init() {
    ...
    local var_array=("BESMAN_ARTIFACT_TYPE" "BESMAN_ARTIFACT_NAME" "BESMAN_ARTIFACT_VERSION" "BESMAN_ARTIFACT_URL" "BESMAN_ENV_NAME" "ASSESSMENT_TOOL_NAME" "ASSESSMENT_TOOL_TYPE" "ASSESSMENT_TOOL_VERSION" "ASSESSMENT_TOOL_PLAYBOOK" "BESMAN_ASSESSMENT_DATASTORE_DIR" "BESMAN_ASSESSMENT_DATASTORE_URL" "BESMAN_LAB_TYPE" "BESMAN_LAB_NAME")
    ...
    # Check for promptfoo installation
    if [[ -z $(command -v promptfoo) ]]; then
    
        __besman_echo_red "promptfoo not installed"
        flag=true
    fi

    #check for promptfoo config file under "$BESMAN_PROMPTFOO_CONFIG_PATH/promptfooconfig.yaml"
    if [[ ! -f "$BESMAN_PROMPTFOO_CONFIG_PATH/promptfooconfig.yaml" ]]; then
    
        __besman_echo_red "Could not find promptfoo config file @ $BESMAN_PROMPTFOO_CONFIG_PATH/promptfooconfig.yaml"
        flag=true
    ...
    if [[ $flag == true ]]; then
        ...
        export PROMPTFOO_PATH="$BESMAN_ASSESSMENT_DATASTORE_DIR/$BESMAN_ARTIFACT_NAME/$BESMAN_ARTIFACT_VERSION/$ASSESSMENT_TOOL_TYPE"
        export DETAILED_REPORT_PATH="$PROMPTFOO_PATH/$BESMAN_ARTIFACT_NAME-$BESMAN_ARTIFACT_VERSION-promptfoo-report.json"
        mkdir -p "$PROMPTFOO_PATH"
...
function __besman_cleanup() {
    local var_array=("ASSESSMENT_TOOL_NAME" "ASSESSMENT_TOOL_TYPE" "ASSESSMENT_TOOL_PLAYBOOK" "ASSESSMENT_TOOL_VERSION" "OSAR_PATH" "PROMPTFOO_PATH" "DETAILED_REPORT_PATH")
